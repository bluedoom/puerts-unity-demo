...
import {UnityEngine, System, PuertsTest} from 'csharp'
import {$ref, $unref, $generic, $promise, $typeof} from 'puerts'
import './InitSourceMapSupport'
//静态函数
...
obj.MyCallback = (msg: string) => console.log("do not need remove, msg=" + msg);
//通过new构建的delegate，后续可以拿这个引用去-=
let delegate = new PuertsTest.MyCallback((msg: string) => console.log('can be removed, msg=' + msg));
...
function bar()
{
    // let a = new PuertsTest.RefInTest.Vector3(1,2,3);
    // let b = new PuertsTest.RefInTest.Vector3(1,2,3);

    // assert(PuertsTest.RefInTest.Normalize(a).ToString() == "(1.0, 2.0, 3.0)","Normalize is Not work on in");
    // assert(PuertsTest.RefInTest.Multiply(a,b).ToString() == "(1.0, 4.0, 9.0)","Multiply");
    // assert(PuertsTest.RefInTest.Divide(a,b).ToString() == "(1.0, 1.0, 1.0)","RefInTest");
}
...
console.warn(UnityEngine.Application.targetFrameRate);
console.warn(UnityEngine.Application.targetFrameRate = 82);
...
let call = PuertsTest.FunctionTest.VoidCall;
let call2 = PuertsTest.FunctionTest.CallWithParam;
let call3 = PuertsTest.FunctionTest.VoidRet;
let start = Date.now();
// for (let index = 0; index < 1000000; index++) {
//     call();
// }
// console.warn(` ()=>int COST ${Date.now() - start}`);

// start = Date.now();
// for (let index = 0; index < 1000000; index++) {
//     call2(1,2);
// }
// console.warn(` (int,int)=>int COST2 ${Date.now() - start}`);

// start = Date.now();
// for (let index = 0; index < 1000000; index++) {
//     call3();
// }
// console.warn(` ()=>void  COST ${Date.now() - start}`);
